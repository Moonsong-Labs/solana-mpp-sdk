...
//! and chain both show the deposit advanced by `additional_amount`.
...
async fn process_topup_advances_chain_deposit_and_store_record() {
    // Open via upstream's `OpenBuilder` so the channel byte layout
    // matches what `recover` will see.
...
async fn extra_instruction_rejects_with_malicious_tx() {
    // Canonical top_up ix plus a system transfer from the fee-payer
    // to an attacker. Without the count gate the server would
    // co-sign and broadcast the smuggled transfer.
    let mut svm = LiteSVM::new();
    svm.add_program_from_file(program_id_address(), program_so_path())
        .expect("load program");
    ...
    config.realm = Some("test".into());
    config.secret_key = Some("test-secret-key-topup-extra-ix".into());
    ...
    let data = resp.value.unwrap().data.decode().unwrap();
    let view = ChannelView::from_account_data(&data).unwrap();
    assert_eq!(view.deposit(), initial_deposit);
}
...
async fn signature_vec_below_required_rejects() {
    ...
    config.realm = Some("test".into());
    config.secret_key = Some("test-secret-key-topup-short-sig".into());
